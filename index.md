...
The variable *D~slr~* represents the number of days since the last rain event. Its value accumulates over time: if the amount of water that is available for infiltration ($W_{av}$) remains below a critical threshold
...
\${O\_{res}} = {O\_{norm}} + \\frac{{(F - {L\_n})}}{{({L\_f} - {L\_n})}}
\\cdot \\max \\{ ({I\_{res}} - {O\_{norm}}),({O\_{nd}} - {O\_{norm}})\\}
& {L\_f} \\ge F \> {L\_n}\$
